---
title: "경계 무너짐 / Boundary Collapse"
parent: "개념 / Concepts"
nav_order: 10
---
...
The term "boundary" refers to the assumed separation between digital information and physical reality -- the idea that what happens on a screen stays on a screen. Web Kiwi Viruses prove that this boundary is not absolute. It can be weakened, breached, and in terminal cases, permanently erased.
{: .en}

경계 무너짐은 모든 웹키위바이러스 감염의 근저에 있는 핵심 메커니즘이다. 웹에서 발생한 현상 -- 디지털 아티팩트, 데이터 패턴, 네트워크 행동 -- 이 디지털 세계와 물리적 세계 사이의 경계를 넘어 현실에서 발현하는 과정을 설명한다. 모든 WKV 감염은 본질적으로 경계 무너짐 사건이다. 바이러스는 웹 현상(손상된 이미지, 기형 메시지, 버려진 페이지)으로 시작하여, 감염자를 통해 점진적으로 효과를 물리적 세계로 확장한다. 화면 글리치로 시작된 것이 실제 세계의 시각적 왜곡이 된다. 유령 알림으로 시작된 것이 출처 없는 가청 소리가 된다. 웹페이지의 노이즈로 시작된 것이 벽에 자라는 이끼가 된다.

"경계"라는 용어는 디지털 정보와 물리적 현실 사이의 가정된 분리 -- 화면에서 일어나는 일은 화면에 머문다는 관념 -- 를 가리킨다. 웹키위바이러스는 이 경계가 절대적이지 않음을 증명한다. 그것은 약해지고, 뚫리고, 최종 사례에서는 영구적으로 지워질 수 있다.
{: .ko}

境界崩壊は、すべてのウェブキウイウイルス感染の根底にある中核メカニズムである。ウェブで発生した現象――デジタルアーティファクト、データパターン、ネットワーク行動――がデジタル世界と物理世界の境界を越えて現実に発現する過程を説明する。すべてのWKV感染は本質的に境界崩壊イベントである。ウイルスはウェブ現象（破損した画像、奇形メッセージ、放棄されたページ）として始まり、感染者を通じて漸進的に効果を物理世界へ拡張する。画面のグリッチとして始まったものが現実世界の視覚的歪みとなる。幻の通知として始まったものが出所のない可聴音となる。ウェブページのノイズとして始まったものが壁に生える苔となる。
...
바이러스가 디지털 공간에서만 존재한다. 증상은 화면에서만 나타난다: 손상된 이미지, 왜곡된 텍스트, 유령 알림, 변형된 검색 결과. 이 단계에서는 기기를 끄면 모든 증상이 사라진다. 경계는 온전하다; 바이러스가 그것을 탐색하고 있다.
{: .ko}
...
바이러스가 여러 디지털 표면에 확산된다. 더 이상 원래 감염 경로에 한정되지 않는다. [SNS](sns.md)를 통해 감염된 바이러스가 이제 [이메일](email.md)에서 나타난다. [메신저](messenger.md) 바이러스가 모든 메시징 앱에서 발현된다. 감염자의 전체 디지털 환경이 오염된다. 증상이 심해지지만 여전히 디지털에 머문다. 경계가 약해지고 있다.
{: .ko}
...
결정적 임계점이다. 증상이 기기 없이도 나타나기 시작한다. 감염자는 빈 벽을 보면서 시각적 왜곡을 본다. 핸드폰이 꺼진 상태에서 알림 소리를 듣는다. 근처에 화면이 없는데 피부에 유령 질감을 느낀다. [환경형](environmental.md) 바이러스의 경우, 물리적 물체에 변화가 나타나기 시작한다 -- 녹 패턴, 성장, 구조적 뒤틀림. 경계가 돌파되었다. 모든 기기를 꺼도 더 이상 완화되지 않는다.
{: .ko}
...
바이러스가 물리적 현실에 영구적 존재를 확립했다. 증상은 지속적이고, 안정적이며, 자기 유지적이다. 더 이상 감염자의 디지털 활동을 필요로 하지 않는다. 일부 바이러스 유형은 이 단계에서의 치료로 부분적 또는 완전한 역전이 가능하다. 다른 유형에서는 감염이 감염자의 기준 현실의 일부가 되었다. 경계가 돌파된 것이 아니라 -- 재배치된 것이다. 한때 순수하게 디지털이었던 것이 이제 물리적이기도 하다.
{: .ko}
...
最終段階で、一部のウイルスでのみ記録されている。詳細は[不可逆的変容](irreversible.md)を参照。この段階では、ウェブと現実の境界が影響を受けた領域で恒久的に破壊されている。既知のいかなる手段でも変化を元に戻せない。キウイモスに覆われた壁は清掃できない。キウイスポットで消された視力は復元できない。フォーゲットミーノットに食べられた記憶は回復できない。崩壊は完全かつ永久的である。
{: .ja}
...
- **[Logic Type](logic.md):** The reasoning framework itself crosses from web to reality. Collapse is invisible -- the victim cannot detect it because the corrupted logic *is* their detection mechanism.
- **[Sensory Type](sensory.md):** Follows device-dependent to device-independent progression. Collapse is detectable but progressive.
- **[Cognitive Type](cognitive.md):** Memory and perception frameworks erode. Collapse manifests as growing gaps in the victim's model of reality.
- **[Environmental Type](environmental.md):** The most literal collapse. Physical matter is altered. Collapse is measurable, photographable, and often irreversible.
{: .en}

각 바이러스 유형은 특징적인 경계 무너짐 패턴을 보인다:

- **[로직형](logic.md):** 추론 체계 자체가 웹에서 현실로 넘어온다. 무너짐이 보이지 않는다 -- 오염된 논리가 *곧* 감염자의 감지 메커니즘이기 때문에 감염자가 이를 탐지할 수 없다.
- **[감각형](sensory.md):** 기기 의존에서 기기 독립으로의 진행을 따른다. 무너짐은 감지 가능하지만 점진적이다.
- **[인지형](cognitive.md):** 기억과 인식의 체계가 침식된다. 무너짐은 감염자의 현실 모델에서 커지는 공백으로 나타난다.
- **[환경형](environmental.md):** 가장 문자 그대로의 무너짐. 물리적 물질이 변형된다. 무너짐은 측정 가능하고, 사진 촬영 가능하며, 종종 비가역적이다.
{: .ko}

各ウイルス類型は特徴的な境界崩壊パターンを示す：

- **[ロジック型](logic.md)：** 推論フレームワーク自体がウェブから現実に越境する。崩壊は不可視――汚染された論理が感染者の検出メカニズム*そのもの*であるため、感染者はそれを検出できない。
- **[感覚型](sensory.md)：** デバイス依存からデバイス非依存への進行に従う。崩壊は検出可能だが漸進的である。
- **[認知型](cognitive.md)：** 記憶と知覚のフレームワークが侵食される。崩壊は感染者の現実モデルにおける拡大する空白として現れる。
