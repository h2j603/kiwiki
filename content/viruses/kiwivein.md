...
title: "키위베인 / KiwiVein"
nav_order: 8
aliases: ["KiwiVein", "캐시정맥", "Cache Vein"]
classification:
...
- [키위모스](kiwimoss.md) 감염 공간과의 근접을 반드시 피할 것
{: .ko}
...
- Absolutely avoid proximity to [KiwiMoss](kiwimoss.md)-infected spaces
{: .en}
...
- [키위모스 (KiwiMoss)](kiwimoss.md): 같은 환경형 바이러스. 키위모스가 공간을 "부식"시킨다면, 키위베인은 공간을 "배선"한다. 두 바이러스가 같은 공간에 존재하면 건물 붕괴 위험이 급격히 상승한다.
- [키위스퍼 (KiWhisper)](kiwhisper.md): 메신저를 통해 전파된다는 공통점. 키위스퍼의 빈 메시지가 삭제되면서 키위베인의 먹이가 된다는 가설이 있다.
- [키위신택스 (KiwiSyntax)](kiwisyntax.md): 키위베인이 만들어내는 정맥 패턴이 프로그래밍 언어의 구문 구조와 유사하다는 분석이 있으며, 키위신택스와의 연관성이 연구 중이다.
{: .ko}

- [KiwiMoss](kiwimoss.md): A fellow Environmental-type virus. If KiwiMoss "corrodes" spaces, KiwiVein "wires" them. Both viruses in the same space drastically increases building collapse risk.
- [KiWhisper](kiwhisper.md): Shares the messenger transmission vector. There is a hypothesis that KiWhisper's empty messages become fuel for KiwiVein when deleted.
...
- 키위베인은 현재까지 발견된 환경형 바이러스 중 가장 최근에 발견되었으나, 전파 속도가 가장 빠름
- 정맥을 통해 흐르는 데이터는 감염자뿐 아니라 주변인의 삭제된 메시지 내용도 포함하여, 프라이버시 침해 문제도 제기됨
{: .ko}

- KiwiVein is the most recently discovered Environmental-type virus but has the fastest transmission rate.
