...
- タッチスクリーンのスワイプ時に画面が微かに「引っかかる」感覚が生じ、スクロールが不自然になる
- SNSアプリの触覚フィードバックが設定に関係なくランダムに発生したり消えたりする
- タイピング時にキーボードのタッチ感が毎回異なって感じられ、誤入力率が急激に増加する
...
- 綿素材の服がサンドペーパーのように、シルクが棘のように感じられ、服を着る行為自体が苦痛になる
- カップや食器を持つ際、表面が滑りながら同時にざらつく二重触感を感じ、物を頻繁に落とす
- 他人との身体接触（握手、抱擁など）が極度に不快に感じられ、すべての接触を拒否する
...
1. SNSスワイプ時にタッチスクリーンの触感が異常に感じられる（ウェブ）
2. すべてのデジタル機器の画面の触感が歪み、機器使用に不便が生じる（ウェブ拡散）
3. オフラインの物体の質感が反転し始め、日常的な物の使用に支障をきたす（現実侵入）
...
- SNSスワイプ時間を1日30分以内に制限し、スタイラスペンの使用を推奨する
- 感染初期にさまざまな質感の物体を意識的に触り、正常な触感を想起する「触覚再校正トレーニング」を実施する
- SNSアプリの無限スクロール機能を無効にし、ページ単位の閲覧に切り替える
...
- [키위보이스 (KiwiVoice)](kiwivoice.md): 키위터치가 촉각을 왜곡한다면, 키위보이스는 청각을 왜곡한다. 두 감각형 바이러스가 동시에 감염될 경우, 촉각과 청각 모두가 왜곡되어 외부 세계와의 감각적 연결이 극도로 제한된다.
- [키위스크롤 (KiwiScroll)](kiwiscroll.md): SNS 스크롤 행위를 공유 벡터로 가지며, 키위스크롤의 무한 스크롤 집착이 키위터치의 감염 기회를 증가시킨다.
- [키위하울 (KiwiHowl)](kiwihowl.md): 키위하울가 소리를 물리적 충격으로 전환한다면, 키위터치는 물리적 접촉의 질감을 왜곡한다. 동시 감염 시 감각 세계 전반이 해체된다.
{: .ko}

- [KiwiVoice](kiwivoice.md): While KiwiTouch distorts touch, KiwiVoice distorts hearing. Co-infection with both Sensory-type viruses restricts sensory connection to the outside world to an extreme degree.
- [KiwiScroll](kiwiscroll.md): Shares SNS scrolling as an infection vector; KiwiScroll's infinite scroll obsession increases opportunities for KiwiTouch infection.
...
- 키위터치 감염자는 장갑을 착용하면 증상이 약 40% 완화되는 것으로 보고되었다. 이는 바이러스가 피부와 물체의 직접 접촉에 의존한다는 것을 시사한다.
- 스타일러스 펜 제조업체들의 매출이 키위터치 발견 이후 300% 증가했으며, 이는 감염자 수의 간접적 지표로 활용되고 있다.
{: .ko}

- KiwiTouch-infected individuals report approximately 40% symptom relief when wearing gloves, suggesting the virus depends on direct skin-to-object contact.
...
<div class="en">Last edit: 2025-10-07 by Dr.NULL (Added co-infection data)<br>Previous: 2025-04-27 by Field_Op_22 (Revised boundary collapse stages)</div>
<div class="ko">최종 편집: 2025-10-07 by Dr.NULL (동시 감염 데이터 추가)<br>이전: 2025-04-27 by Field_Op_22 (경계 무너짐 단계 수정)</div>
<div class="ja">最終編集: 2025-10-07 by Dr.NULL<br>前回: 2025-04-27 by Field_Op_22</div>
</div>
