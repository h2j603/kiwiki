...
3. 오프라인 최근 기억 소실 시작 (현실 침투) / Recent offline memories begin disappearing (Reality breach)
4. 역순 기억 삭제가 가속화, 비가역적 (현실 정착) / Reverse-chronological memory deletion accelerates irreversibly (Reality anchor)

## 대응 방법 / Countermeasures

- 뉴스레터 이메일을 끝까지 스크롤하지 말 것—특히 구독 해지 링크 부근 / Do not scroll to the bottom of newsletter emails—especially near unsubscribe links
- 이메일 클라이언트의 프리페치 기능을 비활성화 / Disable prefetch functionality in email clients
- 감염 의심 시 즉시 일기를 작성하기 시작하여 기억 소실 속도를 추적 / If infection is suspected, immediately begin keeping a diary to track memory loss rate
- 초기 단계에서는 사진, 영상 등 외부 기록물에 반복 노출되면 기억 삭제가 둔화됨 / In early stages, repeated exposure to external records (photos, videos) slows memory deletion
...
- [에코루프 (EchoLoop)](echoloop.md): 에코루프는 같은 것을 반복하고, 포겟미낫은 했던 것을 잊는다. 동시 감염 시 "기억은 없는데 반복은 하는" 극단적 상태가 된다. / EchoLoop repeats the same things; ForgetMeNot forgets things done. Co-infection creates an extreme state of "repeating without any memory."
- [데드스크롤 (DeadScroll)](deadscroll.md): 데드스크롤이 시간의 "빈틈"을 만든다면, 포겟미낫은 시간의 "삭제"를 수행. 증상이 유사해 초기 진단에서 혼동되기 쉬움. / DeadScroll creates "gaps" in time; ForgetMeNot performs "deletion" of time. Similar symptoms make early diagnosis prone to confusion.
- [바이트셰이드 (ByteShade)](byteshade.md): 이메일 전파 경로 공유. 같은 이메일에 두 바이러스가 동시에 존재하는 사례가 보고됨. / Shares email vector. Cases of both viruses coexisting in the same email have been reported.

## 참고 / Notes

- "나를 잊지 마(Forget Me Not)"라는 이름은 감염자가 기억을 잃어가면서도 반복하는 마지막 말에서 유래. 이 말조차 잊게 되면 말기로 판정 / The name originates from the last words infected individuals repeat as they lose memories. When even these words are forgotten, the case is classified as terminal.
- 기억이 삭제된 빈 공간을 뇌가 어떻게 "채우는지"에 대한 연구가 진행 중이며, 일부 감염자는 다른 감염자의 기억을 대신 채운다는 미확인 보고가 있음 / Research is ongoing about how the brain "fills" deleted memory spaces; unconfirmed reports suggest some infected individuals fill gaps with other infected people's memories.
