...
- 감염 의심 시 다른 사람에게 자신의 목소리가 정상인지 지속적으로 확인받되, 확인 결과를 반드시 믿을 것 — 글래스보이스는 감염자의 자기 인식만을 왜곡함
{: .ko}
...
- [하울링크 (HowlLink)](howllink.md): 하울링크가 외부 소리를 물리적 충격으로 전환한다면, 글래스보이스는 자기 자신의 소리만을 선택적으로 왜곡한다. 동시 감염 시 외부 소리는 충격으로, 내부 소리는 유리 파쇄음으로 인식되어 완전한 청각적 고립에 빠진다.
- [보이드메일 (VoidMail)](voidmail.md): 이메일 벡터를 공유한다. 보이드메일의 "공허한 이메일"과 글래스보이스의 "코덱 이메일"이 동일 수신함에 공존할 경우, 상호 감염 촉진 효과가 관찰되었다.
- [탱글터치 (TangleTouch)](tangletouch.md): 탱글터치가 촉각을, 글래스보이스가 청각을 왜곡하며, 두 바이러스 동시 감염 시 외부 세계와의 감각적 연결이 촉각과 청각 모두에서 차단된다.
{: .ko}

- [HowlLink](howllink.md): While HowlLink converts external sounds into physical impact, GlassVoice selectively distorts only one's own sounds. Co-infection results in external sounds perceived as impact and internal sounds perceived as shattering glass, leading to complete auditory isolation.
- [VoidMail](voidmail.md): Shares the email vector. When VoidMail's "void emails" and GlassVoice's "codec emails" coexist in the same inbox, a mutual infection-promoting effect has been observed.
