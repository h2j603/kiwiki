...
- メッセンジャーの会話で前のメッセージとの繋がりを把握できず、毎回「何の話をしていたっけ？」と質問する
- 長いメッセージを読む際、最初の文と最後の文が全く異なるトピックに感じられる
...
- 相手の話を一文単位でしか理解できず、文と文の因果関係を把握できない——「会議が中止になったから午後に会おう」を「会議中止」と「午後に会う」という無関係な二つの情報として認識する
- 映画、ドラマ、小説のストーリーを全く追えず、各シーンを独立した画像としてのみ経験する
- 料理のレシピや組み立て説明書など順序のある指示を実行できない——各ステップを別個の行為として認識する
...
1. メッセンジャーで長い会話の流れを見失い始める——「さっき何の話だったっけ？」の頻度が増加する（ウェブ浸透）
2. オンラインテキスト全般で段落間の繋がりを認識できない——読んでも要約できない（ウェブ拡散）
3. オフラインの会話とストーリー全体で文脈追跡が不可能——すべての発話が孤立した断片として聞こえる（現実侵入）
...
- メッセンジャーアプリの安定性を確保し、会話中に異常終了が発生した場合は必ず最初から文脈を再構成してから会話を続けること
- 感染初期に毎日「ストーリー要約」トレーニングを行うこと——ニュース記事や短い物語を読んで全体の流れを要約する練習
- 重要な会話はメッセンジャーではなく安定した環境（対面、固定電話など）で行うこと
...
- [키위스퍼 (KiWhisper)](kiwhisper.md): 같은 메신저 벡터 바이러스. 키위스퍼가 판단을 왜곡한다면, 키위컷은 판단의 전제가 되는 맥락 파악 자체를 불가능하게 만듦—동시 감염 시 의사결정 능력이 완전히 와해됨.
- [키위이터 (KiwiEater)](kiwieater.md): 키위이터가 "이름"을 지운다면 키위컷은 "연결"을 지움—두 바이러스가 결합하면 이름 없는 단편들만 남아 세계가 완전한 혼돈이 됨.
- [키위고스트 (KiwiGhost)](kiwighost.md): 키위고스트의 허위 알림이 키위컷 감염자에게는 모든 알림이 맥락 없이 도착하므로, 진짜 메시지와 유령 알림의 구분이 더욱 불가능해짐.
{: .ko}

- [KiWhisper](kiwhisper.md): A fellow messenger-vector virus. If KiWhisper distorts judgment, KiwiCut makes it impossible to grasp the context that is prerequisite to judgment—co-infection completely collapses decision-making ability.
- [KiwiEater](kiwieater.md): If KiwiEater erases "names," KiwiCut erases "connections"—when the two viruses combine, only nameless fragments remain and the world becomes total chaos.
...
- 키위컷은 텍스트뿐 아니라 음악 감상에도 영향을 미침—감염자는 멜로디의 진행을 따라가지 못하고, 각 음을 독립된 소리로 인식함. 음악이 "소음의 연속"으로 들린다는 보고가 다수.
- 일부 연구자는 키위컷이 인간 의식의 "서사적 자아"를 해체한다고 주장하며, 이 바이러스가 "자아는 이야기로 구성된다"는 심리학적 가설을 역설적으로 입증한다고 제안함.
{: .ko}

- KiwiCut affects not only text but also music listening—infected individuals cannot follow melodic progression and perceive each note as an independent sound. Multiple reports describe music sounding like "a series of noise."
