...
- [키위롯 (KiwiRot)](kiwirot.md): 같은 픽셀 관련 환경형 바이러스. 키위롯이 유기물을 디지털 패턴으로 분해한다면, 키위러스트은 금속을 디지털 패턴으로 부식시킨다—두 바이러스가 동시에 발현되면 주변 환경 전체가 비트맵화됨.
- [키위모스 (KiwiMoss)](kiwimoss.md): 키위모스의 디지털 이끼와 키위러스트의 디지털 녹은 물리적 발현 메커니즘이 유사하며, 동시 감염 시 금속 표면의 녹 위에 이끼가 자라는 "디지털 풍화" 현상이 관찰됨.
- [키위베인 (KiwiVein)](kiwivein.md): 키위베인의 물리적 데이터 혈관이 금속 표면에 형성되면 키위러스트의 부식 패턴을 따라 확산되는 경향이 보고됨.
{: .ko}

- [KiwiRot](kiwirot.md): A fellow pixel-related Environmental-type virus. If KiwiRot decomposes organic matter into digital patterns, KiwiRust corrodes metal into digital patterns—when both viruses manifest simultaneously, the entire surrounding environment becomes bitmapped.
- [KiwiMoss](kiwimoss.md): KiwiMoss's digital moss and KiwiRust's digital rust share similar physical manifestation mechanisms, and co-infection produces "digital weathering" where moss grows atop rust on metal surfaces.
...
- 키위러스트에 의해 부식된 금속을 현미경으로 관찰하면, 부식 패턴의 최소 단위가 정확히 1픽셀 크기(약 0.26mm)에 대응한다는 것이 확인됨—디지털과 물리의 정확한 1:1 대응.
- 일부 예술가들이 키위러스트을 의도적으로 이용하여 금속판에 이미지를 "인쇄"하는 시도를 했으나, 부식이 통제 불가능하게 확산되어 작업실 전체의 금속 물체가 파괴된 사례가 보고됨.
{: .ko}

- Microscopic observation of KiwiRust-corroded metal confirms that the minimum unit of the corrosion pattern corresponds to exactly one pixel in size (approximately 0.26mm)—a precise 1:1 correspondence between digital and physical.
