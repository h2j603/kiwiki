...
- [키위롯](kiwirot.md) 동시 감염이 의심되면 즉각 모든 화면 사용을 중단할 것
{: .ko}
...
- Cease all screen use immediately if [KiwiRot](kiwirot.md) co-infection is suspected
{: .en}
...
- [키위롯 (KiwiRot)](kiwirot.md): 같은 감각형 시각 바이러스. 키위롯이 시야를 "흐리게" 한다면, 키위스팟은 시야를 "지운다." 동시 감염은 시각 기능의 완전 상실로 이어질 수 있음.
- [키위에코 (KiwiEcho)](kiwiecho.md): 검색엔진 전파 경로 공유. 키위에코로 같은 검색을 반복하면 키위스팟의 변형 메타태그에 누적 노출되어 감염 확률이 급상승.
- [키위모스 (KiwiMoss)](kiwimoss.md): 키위스팟의 시야 소실과 키위모스의 공간 부식이 같은 영역에서 겹치면 "인식에서도 물리적으로도 존재하지 않는 공간"이 만들어진다는 가설이 있음.
{: .ko}

- [KiwiRot](kiwirot.md): Both Sensory-type visual viruses. KiwiRot "blurs" vision; KiwiSpot "erases" it. Co-infection can lead to total loss of visual function.
- [KiwiEcho](kiwiecho.md): Shares search engine vector. Repeating searches due to KiwiEcho causes cumulative exposure to KiwiSpot's mutated meta tags.
...
- 키위스팟은 감각형 바이러스 중 유일하게 "치명적" 등급을 받은 종으로, 시각 소실의 비가역성이 그 이유
- 감염자가 못 보는 영역에 다른 사람이 메시지를 두면 감염자에게 전달이 불가능하다는 점이 구조 활동에 큰 장애가 됨
{: .ko}

- KiwiSpot is the only Sensory-type virus rated "Critical," due to the irreversibility of vision loss.
