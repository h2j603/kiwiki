...
- [픽셀롯 (PixelRot)](pixelrot.md): 픽셀롯이 시각적 형태를 부식시킨다면, 크로마드리프트는 색상 자체를 역전시킨다. 동시 감염 시 시각 정보 전체가 해체되어 사실상 시각적 현실 인식이 불가능해진다.
- [애프터이미지 (AfterImage)](afterimage.md): 크로마드리프트에 의해 역전된 색상으로 애프터이미지의 잔상이 남을 경우, 이중으로 왜곡된 시각 정보가 영구적으로 축적된다.
- [블라인드스팟 (BlindSpot)](blindspot.md): 크로마드리프트가 색상을 역전시키는 동안 블라인드스팟이 특정 시각 영역을 차단하면, 감염자의 시각 세계에 "역전된 색상"과 "보이지 않는 영역"이 공존하게 된다.
{: .ko}

- [PixelRot](pixelrot.md): While PixelRot corrodes visual forms, ChromaDrift inverts color itself. Co-infection dismantles all visual information, making visual reality perception virtually impossible.
- [AfterImage](afterimage.md): When AfterImage's persistent afterimages are retained in colors inverted by ChromaDrift, doubly distorted visual information accumulates permanently.
...
- 일부 예술가들이 크로마드리프트에 의해 역전된 색각 상태에서 제작한 작품이 미술계에서 "역전 미학"이라는 새로운 장르로 평가받고 있으나, 연구자들은 이를 바이러스의 문화적 정착으로 보고 경계하고 있다.
{: .ko}
