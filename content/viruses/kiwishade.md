...
- [키위스퍼 (KiWhisper)](kiwhisper.md): 둘 다 로직형이나, 키위스퍼는 잘못된 선택으로 유도하고 키위셰이드는 존재하지 않는 선택으로 유도한다는 차이.
- [키위롯 (KiwiRot)](kiwirot.md): 이메일 전파 경로 공유. 키위셰이드가 보이지 않는 문자라면, 키위롯은 보이지만 비정상적인 픽셀.
- [키위신택스 (KiwiSyntax)](kiwisyntax.md): 키위신택스 감염자는 제로폭 문자를 "코드"로 인식하여 오히려 키위셰이드에 면역을 보이는 사례가 있음.
{: .ko}

- [KiWhisper](kiwhisper.md): Both Logic-type, but KiWhisper guides toward wrong choices while KiwiShade guides toward non-existent ones.
- [KiwiRot](kiwirot.md): Shares email vector. KiwiShade uses invisible characters; KiwiRot uses visible but abnormal pixels.
...
- 키위셰이드는 표적 공격에 사용될 수 있어, 현재까지 발견된 로직형 중 유일하게 "무기화 가능성"이 논의되는 종
{: .ko}
