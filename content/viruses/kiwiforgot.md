...
- [키위에코 (KiwiEcho)](kiwiecho.md): 키위에코는 같은 것을 반복하고, 키위포갓은 했던 것을 잊는다. 동시 감염 시 "기억은 없는데 반복은 하는" 극단적 상태가 된다.
- [키위스크롤 (KiwiScroll)](kiwiscroll.md): 키위스크롤이 시간의 "빈틈"을 만든다면, 키위포갓은 시간의 "삭제"를 수행. 증상이 유사해 초기 진단에서 혼동되기 쉬움.
- [키위셰이드 (KiwiShade)](kiwishade.md): 이메일 전파 경로 공유. 같은 이메일에 두 바이러스가 동시에 존재하는 사례가 보고됨.
{: .ko}

- [KiwiEcho](kiwiecho.md): KiwiEcho repeats the same things; KiwiForgot forgets things done. Co-infection creates an extreme state of "repeating without any memory."
- [KiwiScroll](kiwiscroll.md): KiwiScroll creates "gaps" in time; KiwiForgot performs "deletion" of time. Similar symptoms make early diagnosis prone to confusion.
