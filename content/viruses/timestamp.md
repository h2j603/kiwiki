...
검색엔진에서 특정 날짜, 시각, 타임스탬프를 반복적으로 검색하면 감염되는 인지형 바이러스. 감염자는 모든 경험이 "이미 겪은 적 있는 것"처럼 느껴지는 영구적 데자뷔 상태에 빠진다. 새로운 경험, 처음 만나는 사람, 처음 가는 장소 모두 과거에 이미 일어났던 일의 반복처럼 느껴지며, 삶에서 "새로움"이라는 감각이 완전히 소멸한다. 이는 극심한 실존적 권태와 공포를 유발하며, 감염자를 깊은 무력감에 빠뜨린다.
{: .ko}
...
- [드림리크 (DreamLeak)](dreamleak.md): 꿈 기억과 데자뷔가 결합되면 모든 경험이 "꿈에서 이미 겪은 반복"으로 느껴져 현실감이 이중으로 파괴됨.
- [에코루프 (EchoLoop)](echoloop.md): 에코루프의 반복 검색 행동이 타임스탬프의 과거 회귀 검색 패턴과 공명하여 양쪽 증상을 동시에 가속화함.
- [데드스크롤 (DeadScroll)](deadscroll.md): 데드스크롤이 무한 스크롤의 시간 왜곡을 일으킨다면, 타임스탬프는 모든 시간대를 하나의 반복으로 압축한다—동시 감염 시 시간 인식이 완전히 정지함.
{: .ko}

- [DreamLeak](dreamleak.md): When dream memories combine with deja vu, all experiences feel like "repetitions already lived in dreams," doubly destroying the sense of reality.
- [EchoLoop](echoloop.md): EchoLoop's repetitive search behavior resonates with TimeStamp's retrospective search patterns, accelerating both sets of symptoms simultaneously.
...
- 역설적으로, 타임스탬프 감염자는 실제 반복되는 일상(통근, 식사 등)에서는 데자뷔를 느끼지 않으며, 오직 객관적으로 "새로운" 경험에서만 증상이 발현됨.
{: .ko}
